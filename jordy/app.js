...
const materialsDemo = {
  plant: {
    ...
    pot: new THREE.MeshStandardMaterial({
      map: new THREE.TextureLoader().load(
        "./objects/plants/textures/indoor plant_2_NOR.jpg",
      ),
    }),
  ...
  can: {
    base: new THREE.MeshStandardMaterial({
      map: new THREE.TextureLoader().load(
        "./objects/can/CokeCan_Base_color.png",
      ),
    }),
    top: new THREE.MeshStandardMaterial({
      map: new THREE.TextureLoader().load("textures/can_top.jpg"),
    }),
  },
  car: {
    base: new THREE.MeshStandardMaterial({
      map: new THREE.TextureLoader().load("./objects/car/car.jpg"),
    }),
    top: new THREE.MeshStandardMaterial({
      map: new THREE.TextureLoader().load("textures/can_top.jpg"),
    }),
...
function startRecording() {
  ...
  console.log(
    "Recording started. Move the camera and press 'L' to lock positions.",
  )
}
...
const cameraPositions = {
  default: {
    position: new THREE.Vector3(0, 150, 300),
...
window.addEventListener("keydown", function (event) {
  ...
  if (event.key === "l" || event.key === "L") {
    ...
    if (event.shiftKey && event.altKey) {
      currentColorIndexMain =
        (currentColorIndexMain + 1) % spotlightColors.length

      mainSpotLight.color.setHex(spotlightColors[currentColorIndexMain])
    } else if (event.altKey && !event.shiftKey) {
      currentColorIndexLeft =
        (currentColorIndexLeft + 1) % spotlightColors.length
      leftSpotLight.color.setHex(spotlightColors[currentColorIndexLeft])
    } else if (!event.altKey && event.shiftKey) {
      currentColorIndexRight =
        (currentColorIndexRight + 1) % spotlightColors.length
  ...
  switch (event.key) {
    case "1":
      moveCameraTo(
        cameraPositions.default.position,
        cameraPositions.default.lookAt,
      )
      break
    case "2":
      moveCameraTo(
        cameraPositions.interesting1.position,
        cameraPositions.interesting1.lookAt,
      )
      break
    case "3":
      moveCameraTo(
        cameraPositions.interesting2.position,
        cameraPositions.interesting2.lookAt,
      )
      break
    case "4":
